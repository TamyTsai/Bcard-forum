...
export default class extends Controller {
    static targets = [ "followButton", "followButtonBig" ]
    ...
    followBig(event) {
        ...
        let target = this.followButtonBigTarget
        // 要被變動顯示內容 的html元素
            ...
            .then(function(response){
                ...
                switch (status) {
                    ...
                    case '已取消追蹤':
                        target.innerHTML = '追蹤'; // innerHTML對html元素內容的置換可以及時顯示，不用重新整理頁面
                        // alert('已取消追蹤');
                        target.classList.add('bgc-light-blue', 'hover:bg-sky-400', 'text-white');
                        target.classList.remove('bg-gray-300', 'hover:bg-white');
                        break;
                    case '已追蹤':
                        target.innerHTML = '追蹤中';
                        target.classList.add('bg-gray-300', 'hover:bg-white');
                        target.classList.remove('bgc-light-blue', 'hover:bg-sky-400', 'text-white');
                        break;
                }
            })
            .catch(function(error){
